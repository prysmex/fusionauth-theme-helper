    ...
    document.addEventListener('DOMContentLoaded', () => {  
      ...
      if (clientId) {
        ...
        if(tenantStr) {
          ...
          if (tenantBadge) {
            tenantBadge.innerHTML = '<span style="margin: 8px 0px 8px 0px; display: inline-block;" class="euiBetaBadge euiBetaBadge--hollow">' + tenantStr + '</span>';
          }
        }
      }
...
<main class="euiPageBody container">
  [@printErrorAlerts rowClass colClass/]
  [@printInfoAlerts rowClass colClass/]
  <div class="${rowClass}">
    <div class="${colClass}">
...
<div class="euiPanel euiPanel--hasShadow euiPanel--paddingLarge euiFlexGroup euiFlexGroup--directionColumn euiFlexGroup--gutterMedium">
  [#if title?has_content]
    <h2 class="euiTitle euiTitle--large euiFlexItem">${title}</h2>
  [/#if]
  <main>
...
[#macro _input_text type name id autocapitalize autocomplete autocorrect autofocus spellcheck label placeholder leftAddon required tooltip disabled  dateTimeFormat class="euiFieldText--fullWidth euiFieldText" ]
